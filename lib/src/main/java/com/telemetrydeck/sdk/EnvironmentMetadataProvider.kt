...
class EnvironmentMetadataProvider : TelemetryProvider {
    ...
    override fun register(ctx: Application?, manager: TelemetryManager) {
        ...
        if (android.os.Build.VERSION.RELEASE.isNullOrEmpty()) {
            ...
            if (sdkVersion < 24) {
                val systemVersionArray = release.split(".")
                val major = systemVersionArray[0]
                val minor = systemVersionArray[1]

                metadata["majorSystemVersion"] = major
                metadata["majorMinorSystemVersion"] =  "$major.$minor"
            } else {
